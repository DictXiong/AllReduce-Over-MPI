...
// 单纯的发送, 可通用
void handle_send(std::vector<Operation> *ops, DataType *data, size_t len, size_t num_split)
{
    CHECK_NOTNULL(ops);
    CHECK_NOTNULL(data);
...
// 接收后, 还负责加和
void handle_recv_gather(std::vector<Operation> *ops, DataType *data, size_t len, size_t num_split)
{
    CHECK_NOTNULL(ops);
    CHECK_NOTNULL(data);
...
// 接收后, 还负责直接覆盖对应数据块的数据
void handle_recv_broadcast(std::vector<Operation> *ops, DataType *data, size_t len, size_t num_split)
{
    CHECK_NOTNULL(ops);
    CHECK_NOTNULL(data);
...
{
    CHECK_NOTNULL(data);
    CHECK_EQ(0, len % (total_peers - num_lonely)) << "data length should be an integral multiple of the total bumber of nodes";

    size_t num_split = num_nodes - num_lonely;
    //LOG_IF(WARNING, node_label == 0) << "gathering start";
    Send_Ops send_ops(total_peers, num_lonely, node_label, stages);
    Recv_Ops recv_ops(total_peers, num_lonely, node_label, stages);
    ...
    if (node_label < num_split)
    {
        ...
        if (num_lonely != 0)
        {
            lonely_thread = new std::thread(handle_recv_broadcast, &(recv_ops.lonely_ops), data, len, num_split);
            MPI_Comm_split(MPI_COMM_WORLD, 12, node_label, sub_comm); // 这个 12 是 magic number, 用来标注本组的颜色.
        }
        for (size_t i = 0; i != stages.size(); i++)
        {
            std::thread send_thread(handle_send, &(send_ops.ops[i]), data, len, num_split);
            std::thread recv_thread(handle_recv_gather, &(recv_ops.ops[i]), data, len, num_split);
            send_thread.join();
            recv_thread.join();
        ...
        if (num_lonely != 0)
        {
            lonely_thread = new std::thread(handle_send, &(recv_ops.lonely_ops), data, len, num_split);
        }
        //LOG_IF(WARNING, node_label == 0) << "gathering done";
        for (int i = stages.size() - 1; i >= 0; i--)
        {
            std::thread send_thread(handle_send, &(recv_ops.ops[i]), data, len, num_split);
            std::thread recv_thread(handle_recv_broadcast, &(send_ops.ops[i]), data, len, num_split);
            send_thread.join();
            recv_thread.join();
            MPI_Barrier(*sub_comm);
        }
    ...
    {
        handle_send(&(send_ops.lonely_ops), data, len, num_split);
        MPI_Barrier(MPI_COMM_WORLD);
        handle_recv_broadcast(&(send_ops.lonely_ops), data, len, num_split);
        MPI_Barrier(MPI_COMM_WORLD);
    }
    //LOG_IF(WARNING, node_label == 0) << "broadcast done";
}

//要改! num_split //这是ring改个锤子!
void ring_allreduce(DataType *data, size_t len, size_t num_nodes, size_t node_label)
{
    ...
    for (size_t i = 0; i != num_nodes - 1; i++)
    {
        ...
        std::thread send_thread(handle_send, &send_ops, data, len, num_nodes);
        std::thread recv_thread(handle_recv_gather, &recv_ops, data, len, num_nodes);
        send_thread.join();
        recv_thread.join();
        MPI_Barrier(MPI_COMM_WORLD);
    ...
    for (size_t i = 0; i != num_nodes - 1; i++)
    {
        ...
        std::vector<Operation> recv_ops = {Operation(left, block_recv)};
        std::thread send_thread(handle_send, &send_ops, data, len, num_nodes);
        std::thread recv_thread(handle_recv_broadcast, &recv_ops, data, len, num_nodes);
        send_thread.join();
        recv_thread.join();
        MPI_Barrier(MPI_COMM_WORLD);
        block_send = (block_send == 0 ? num_nodes - 1 : block_send - 1);
