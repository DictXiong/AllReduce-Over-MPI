...
#define TIME_RESET()              \
    do                            \
    {                             \
        _time_base = MPI_Wtime(); \
    } while (false)
#define TIME_LOG_IF(exp, note)                                           \
    do                                                                   \
    {                                                                    \
        LOG_IF(INFO, exp) << MPI_Wtime() - _time_base << " :: " << note; \
    } while (false)
#endif
// end of LOG 控制
...
namespace FlexTreeNCCL
{
    ...
    public:
        ...
        std::vector<size_t> find_followers(size_t h, size_t n = INF) const
        {
            ...
            {
                if (find_star(i) % gap == n % gap)
                    ans.push_back(i);
            }
            if (has_lonely_blocks(h, n))
                return ans;
            else
                return {};
        }
    ...
    public:
        ...
        virtual void print() const
        {
            std::cout << "\033[0m\033[1;30;42m" << typeid(*this).name() << "\033[0m"
                      << " of node " << raw_send_operations->node_label << " in total " << raw_send_operations->num_split << " peers: " << std::endl;
    ...
    __global__ void reduce_sum_1(const DataType *src0, DataType *dst, const size_t num_elements)
    {
    ...
    template <class DataType>
    __global__ void reduce_sum_2(const DataType *src0, const DataType *src1, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_3(const DataType *src0, const DataType *src1, const DataType *src2, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_4(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_5(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_6(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, const DataType *src5, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_7(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, const DataType *src5, const DataType *src6, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_8(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, const DataType *src5, const DataType *src6, const DataType *src7, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_9(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, const DataType *src5, const DataType *src6, const DataType *src7, const DataType *src8, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_10(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, const DataType *src5, const DataType *src6, const DataType *src7, const DataType *src8, const DataType *src9, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_11(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, const DataType *src5, const DataType *src6, const DataType *src7, const DataType *src8, const DataType *src9, const DataType *src10, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    template <class DataType>
    __global__ void reduce_sum_12(const DataType *src0, const DataType *src1, const DataType *src2, const DataType *src3, const DataType *src4, const DataType *src5, const DataType *src6, const DataType *src7, const DataType *src8, const DataType *src9, const DataType *src10, const DataType *src11, DataType *dst, const size_t num_elements)
    {
        int i = blockDim.x * blockIdx.x + threadIdx.x;
        if (i < num_elements)
        {
    ...
    static void handle_send(ncclComm_t comm, ncclDataType_t datatype, const std::vector<FMA_Operation> *fma_ops, const void *data, const FlexTree_Context &ft_ctx, cudaStream_t stream)
    {
        ...
        for (const auto &i : *fma_ops)
        {
            if (LIKELY(ft_ctx.node_label != i.peer))
            {
                ...
                {
#ifdef FT_DEBUG
                    if (ft_ctx.node_label == 8 || false)
                        std::cout << ft_ctx.node_label << " send " << j.to_string() << " to " << i.peer << ", element size = " << ft_ctx.type_size << std::endl;
#endif
                    if (j.len > 0)
                    {
                        //MPI_Isend(data + j.addr * ft_ctx.type_size, j.len, datatype, i.peer, 0, comm, &request[request_index++]); // 此处的tag暂时先打0
                        NCCLCHECK(ncclSend(data + j.addr * ft_ctx.type_size, j.len, datatype, i.peer, comm, stream));
                    }
                }
            }
        }
    ...
    static void handle_recv(ncclComm_t comm, ncclDataType_t datatype, const std::vector<FMA_Operation> *fma_ops, void *buffer, const FlexTree_Context &ft_ctx, cudaStream_t stream)
    {
        for (const auto &i : *fma_ops)
        {
            if (LIKELY(ft_ctx.node_label != i.peer))
            {
                for (const auto &j : i.ranges)
                {
                    ...
                    if (j.len > 0)
                    {
                        ...
                        NCCLCHECK(ncclRecv(buffer + j.addr * ft_ctx.type_size, j.len, datatype, i.peer, comm, stream));
                    }
                }
            }
        }
        ...
        if (ft_ctx.node_label == 0)
            std::cout << "REQ INDEX = " << request_index << std::endl;
#endif
    }
     ...
     */
    static void handle_reduce(const ncclDataType_t &datatype, const ncclRedOp_t &op, const std::vector<FMA_Operation> *fma_ops, void *buffer, const void *data, void *dest, const FlexTree_Context &ft_ctx)
    {
        if (dest == nullptr)
        {
        ...
        for (int i = 0; i < num_blocks; i++)
        {
...
#ifdef FT_DEBUG
                if (ft_ctx.node_label == 0)
                    std::cout << ft_ctx.node_label << " will not reduce " << fma_ops->begin()->ranges[i].to_string() << " because it's empty." << std::endl;
#endif
...
#ifdef FT_DEBUG
                if (ft_ctx.node_label == 8)
                    std::cout << "WATCH == :" << ((float *)src[block_index - 1])[2] << std::endl;
#endif
            }
            ...
            if (ft_ctx.node_label == 8)
                if (ft_ctx.node_label == 8)
                    std::cout << ft_ctx.node_label << " call reduce_sum, #blocks = " << std::endl;
#endif
            // cuda config
            uint threadsPerBlock = 256;
            uint blocksPerGrid = (block_len + threadsPerBlock - 1) / threadsPerBlock;
            if (op == ncclSum)
            {
                if (datatype == ncclUint8)
                    reduce_sum_gpu((const uint8_t **)src, (uint8_t *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else if (datatype == ncclInt8 || datatype == ncclChar)
                    reduce_sum_gpu((const int8_t **)src, (int8_t *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else if (datatype == ncclInt32 || datatype == ncclInt)
                    reduce_sum_gpu((const int32_t **)src, (int32_t *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else if (datatype == ncclUint32 || datatype == ncclUint32)
                    reduce_sum_gpu((const uint32_t **)src, (uint32_t *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else if (datatype == ncclInt64)
                    reduce_sum_gpu((const int64_t **)src, (int64_t *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else if (datatype == ncclUint64)
                    reduce_sum_gpu((const uint64_t **)src, (uint64_t *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else if (datatype == ncclFloat32 || datatype == ncclFloat)
                    reduce_sum_gpu((const float **)src, (float *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else if (datatype == ncclFloat64 || datatype == ncclDouble)
                    reduce_sum_gpu((const double **)src, (double *)dst, block_index, block_len, blocksPerGrid, threadsPerBlock);
                else
                {
                    char name[20];
                    int name_len;
        ...
        else
        {
            ...
            while (!ss.eof())
            {
                ...
                if (tmp == 1)
                {
                    ...
                    // I REALLY don't know why this cannot work. 
...
#endif
            CUDACHECK(cudaMalloc(&buffer, _size));
            size = _size;
        }
        return buffer;
    }
    ...
    static void tree_allreduce(const ncclDataType_t &datatype, const ncclRedOp_t &op, const MPI_Comm &mpi_comm, const ncclComm_t &nccl_comm, const void *data, void *dst, const FlexTree_Context &ft_ctx, const std::vector<size_t> &stages, cudaStream_t stream)
    {
#ifdef FT_DEBUG
        ...
        CUDACHECK(cudaStreamCreate(&send_stream));
        CUDACHECK(cudaStreamCreate(&recv_stream));
        if (data == nullptr)
        {
            data = dst;
        }
        ...
        {
            MPI_Comm_split(mpi_comm, 0, ft_ctx.node_label, &sub_comm);
        }
        else
        {
            MPI_Comm_split(mpi_comm, 1, ft_ctx.node_label, &sub_comm);
        }
        ...
        for (size_t i = 0; i < num_stages; i++)
        {
            ...
            {
                handle_send(nccl_comm, datatype, &(fma_send_ops.FMA_ops[i]), fma_send_ops.FMA_ops[i].begin()->from_src ? data : dst, ft_ctx, send_stream);
            }
            // 另: 能不能想办法把这些 if 去掉? 或许可以在 handle_** 里面加一些判断?
            if (!fma_send_ops.FMA_lonely_ops[i].empty())
            {
                handle_send(nccl_comm, datatype, &(fma_send_ops.FMA_lonely_ops[i]), fma_send_ops.FMA_lonely_ops[i].begin()->from_src ? data : dst, ft_ctx, send_stream);
            }
            NCCLCHECK(ncclGroupEnd());
            NCCLCHECK(ncclGroupStart());
            if (!fma_recv_ops.FMA_ops.empty())
            {
                handle_recv(nccl_comm, datatype, &(fma_recv_ops.FMA_ops[i]), recv_buffer, ft_ctx, recv_stream);
            }
            if (!fma_recv_ops.FMA_lonely_ops[i].empty())
            {
                handle_recv(nccl_comm, datatype, &(fma_recv_ops.FMA_lonely_ops[i]), recv_buffer, ft_ctx, recv_stream);
            }
            NCCLCHECK(ncclGroupEnd());
            ...
            cudaStreamSynchronize(recv_stream);
                ...
                if (ft_ctx.node_label == 0)
                    std::cout
                << "HERE AND: " << tmp << ", recv_buffer[15] = " << ((float *)(recv_buffer))[15] << std::endl;
#endif
            if (!fma_recv_ops.FMA_ops.empty())
            {
            ...
            MPI_Barrier(i == 0 || i == num_stages - 1 ? mpi_comm : sub_comm);
        }
#ifdef SHOW_TIME
...
// LOG_IF(WARNING, node_label == 0) << "gathering done";
        ...
        for (int i = 0; i <= ft_ctx.num_nodes; i++)
        {
            ...
            if (i == ft_ctx.node_label + 1)
            {
                ...
                for (int i = 0; i < std::min(100UL, ft_ctx.data_size); i++)
        ...
        for (int i = num_stages; i < (num_stages << 1); i++)
        {
            ...
            if (!fma_send_ops.FMA_ops.empty())
            {
                handle_send(nccl_comm, datatype, &(fma_send_ops.FMA_ops[i]), dst, ft_ctx, stream);
            }
            // if (ft_ctx.node_label == 0) std::cout << "***FUCK: " << fma_send_ops.FMA_ops[i].begin()->to_string() << std::endl;
            if (!fma_send_ops.FMA_lonely_ops[i].empty())
            {
                handle_send(nccl_comm, datatype, &(fma_send_ops.FMA_lonely_ops[i]), dst, ft_ctx, stream);
            }
            if (!fma_recv_ops.FMA_ops.empty())
            {
                handle_recv(nccl_comm, datatype, &(fma_recv_ops.FMA_ops[i]), dst, ft_ctx, stream);
            }
            // if (ft_ctx.node_label == 0) std::cout << "***FUCK: " << fma_recv_ops.FMA_ops[i].begin()->to_string() << std::endl;
            if (!fma_recv_ops.FMA_lonely_ops[i].empty())
            {
                handle_recv(nccl_comm, datatype, &(fma_recv_ops.FMA_lonely_ops[i]), dst, ft_ctx, stream);
            }
            NCCLCHECK(ncclGroupEnd());
            cudaStreamSynchronize(stream);
            MPI_Barrier(i == num_stages || i == (num_stages << 1) - 1 ? mpi_comm : sub_comm);
        }
#ifdef SHOW_TIME
...
// std::cout << "WHY HERE: " << ((int32_t*)dst)[9] << " " << ((int32_t*)dst)[12] << std::endl;
    ...
    static void ring_allreduce(const ncclDataType_t &datatype, const ncclRedOp_t &op, const MPI_Comm &mpi_comm, const ncclComm_t nccl_comm, const void *data, void *dst, const FlexTree_Context &ft_ctx, cudaStream_t stream)
    {
        if (data == nullptr)
        {
            data = dst;
        ...
        for (size_t i = 0; i != ft_ctx.num_nodes - 1; i++)
        {
            ...
            NCCLCHECK(ncclGroupStart());
            handle_send(nccl_comm, datatype, &fma_send_ops, (i == 0 ? data : dst), ft_ctx, stream);
            handle_recv(nccl_comm, datatype, &fma_recv_ops, recv_buffer, ft_ctx, stream);
            NCCLCHECK(ncclGroupEnd());
            cudaStreamSynchronize(stream);
        ...
        for (size_t i = 0; i != ft_ctx.num_nodes - 1; i++)
        {
            ...
            NCCLCHECK(ncclGroupStart());
            handle_send(nccl_comm, datatype, &fma_send_ops, dst, ft_ctx, stream);
            handle_recv(nccl_comm, datatype, &fma_recv_ops, dst, ft_ctx, stream);
            NCCLCHECK(ncclGroupEnd());
            cudaStreamSynchronize(stream);
            MPI_Barrier(mpi_comm);
            block_send = (block_send == 0 ? ft_ctx.num_nodes - 1 : block_send - 1);
...
int ncclAllReduce_FT(const void *sendbuf, void *recvbuf, size_t count, ncclDataType_t datatype, ncclRedOp_t op, ncclComm_t comm, cudaStream_t stream, MPI_Comm mpi_comm)
#else
static int ncclAllReduce(const void *sendbuf, void *recvbuf, int count, ncclDataType_t datatype, ncclRedOp_t op, ncclComm_t comm, cudaStream_t stream)
#endif
{
#ifdef FT_DEBUG
    std::cout << "FlexTree AR called" << std::endl;
#endif
    std::pair<std::vector<size_t>, size_t> topo = FlexTreeNCCL::get_stages(FlexTreeNCCL::FlexTree_Context::get_num_nodes(mpi_comm));
    auto stages = topo.first;
    const FlexTreeNCCL::FlexTree_Context ft_ctx(mpi_comm, datatype, count, topo.second);
#ifdef FT_DEBUG
    if (ft_ctx.node_label == 0)
        ft_ctx.show_context();
#endif
    ...
    if (ft_ctx.num_nodes <= 1)
    {
        ...
        {
            CUDACHECK(cudaMemcpy(recvbuf, sendbuf, count * ft_ctx.type_size, cudaMemcpyDeviceToDevice));
        }
        return 0;
    }

    FlexTreeNCCL::recv_buffer = FlexTreeNCCL::flextree_register_the_buffer((ft_ctx.data_size_aligned * ft_ctx.type_size) << 1);
    ...
    if (stages[0] != 1)
    {
        if (sendbuf == MPI_IN_PLACE)
        {
            FlexTreeNCCL::tree_allreduce(datatype, op, mpi_comm, comm, nullptr, recvbuf, ft_ctx, stages, stream);
        }
        else
        {
            FlexTreeNCCL::tree_allreduce(datatype, op, mpi_comm, comm, sendbuf, recvbuf, ft_ctx, stages, stream);
        }
    }
    else
    {
        if (sendbuf == MPI_IN_PLACE)
        {
            FlexTreeNCCL::ring_allreduce(datatype, op, mpi_comm, comm, nullptr, recvbuf, ft_ctx, stream);
        }
        else
        {
            FlexTreeNCCL::ring_allreduce(datatype, op, mpi_comm, comm, sendbuf, recvbuf, ft_ctx, stream);
        }
    }
